...
try:
    ...
    __version__ = _get_version("quicksand-ubuntu-desktop")
except Exception:
    __version__ = "0.0.0"
...
class _UbuntuDesktopImageProvider:
    ...
    def resolve(self, arch: str | None = None):
        ...
        save_dir = resolve_dir("quicksand-ubuntu-desktop", _IMAGES_DIR)
        if save_dir is None:
            from quicksand_core._auto_install import auto_install_images

            if not auto_install_images("quicksand-ubuntu-desktop", _IMAGES_DIR):
                raise FileNotFoundError(
                    "No bundled save found. If you installed from PyPI, download images with:\n"
                    "  quicksand install quicksand-ubuntu-desktop"
                )
            save_dir = resolve_dir("quicksand-ubuntu-desktop", _IMAGES_DIR)
            if save_dir is None:
